...
export default function GridScan({
  ...
  const pushScan = (t: number) => {
    ...
    if (materialRef.current) {
      ...
      for (let i = 0; i < arr.length && i < maxScans; i++) {
        buf[i] = arr[i];
      }
  ...
  useEffect(() => {
    ...
    const target = listenToWindow ? window : el;
  ...
  }, [scanOnClick, snapBackDelay, listenToWindow]);
  ...
  useEffect(() => {
    ...
    const renderer = new THREE.WebGLRenderer({
      antialias: false, // Matikan antialias - shader grid tidak membutuhkannya, hemat GPU
      alpha: true
    });

    rendererRef.current = renderer;
    // Kurangi pixel ratio max dari 2 → 1.5 = fragment shader bekerja 25% lebih sedikit
    ...
    const tick = () => {
      if (!isVisible) {
        // Pause rendering ketika tidak terlihat — hemat GPU signifikan
        rafRef.current = null;
        return;
      }
    ...
    >
      {showPreview && enableWebcam && (
        <div className="gridscan__preview">
          <video muted playsInline autoPlay className="gridscan__video" />
          <div className="gridscan__badge">Webcam disabled</div>
        </div>
      )}
    </div>
  );
